...
class seocopy_Admin
{
    ...
    public function seo_copy_redirect(){
        if (get_option('seo_copy_do_activation_redirect', false)) {
    ...
    public function seo_copy_settings(){
        add_submenu_page( 'wp-seo-plugins-login', 'Copy', 'Copy', 'manage_options', 'seo-copy', array($this, "seo_copy_settings_view") );
    }

    public function seo_copy_settings_view(){
        include_once 'partials/seocopy-admin-display.php';
    }
...
//            'get_test_links' );
    ...
    function getApiLanguage(){
        ...
        $option = get_option( 'seocopy_language' );
        if( !empty( $option ) ){
            $curlang = $option;
        }
